...
import { ArrowRight, CircleHelp, Inbox, Settings, Users } from 'lucide-react';
import { ScrollArea } from '@/components/ui/scroll-area';
import { Badge } from '@/components/ui/badge';
import StudentClassesTodayItem from '@/components/(authenticated)/student/dashboard/student-classes-today-item';
import { IClass } from '@/types/models';

interface IStudentClassesTodayProps {
  classes: IClass[];
}

const StudentClassesToday = ({ classes }: IStudentClassesTodayProps) => {
  return (
      ...
      <CardHeader className="flex items-center gap-2 space-y-0 py-5 sm:flex-row">
        ...
        <CardAction>
          <Button asChild size="sm" variant="outline">
            <Link href="/student/reports/class">
        ...
        <ScrollArea className="h-80">
          {classes.length > 0 ? (
            classes.map((classItem, index) => (
              <StudentClassesTodayItem key={index} classItem={classItem} />
            ))
          ) : (
            <div
...
export default StudentClassesToday;
