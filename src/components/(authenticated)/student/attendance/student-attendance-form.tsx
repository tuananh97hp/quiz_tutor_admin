...
import { StudentAttendanceStudentDataTable } from '@/components/(authenticated)/student/attendance/student-attendance-student-table';
import { checkUpcomingOrStarted, currentScheduleOfDate } from '@/utils/models';

interface IStudentAttendanceFormProps {
  classItem: IClass;
}

export const StudentAttendanceForm = ({ classItem }: IStudentAttendanceFormProps) => {
  ...
  useEffect(() => {
    startTransitionPending(async () => {
      if (currentSession?.accessToken && classItem.id) {
        const results = await AttendanceService.studentMeGetStudentAttendanceOfClass(
  ...
  function onSubmit() {
    startTransitionSubmit(async () => {
      ...
      try {
        ...
        await AttendanceService.studentMeUpdateStudentAttendance(
          currentSession?.accessToken,
          classItem.id,
          {
          ...
          <Card className="relative border-2 rounded-xl before:rounded-xl bg-background">
            ...
            <CardContent className="p-2 pb-5">
              <div className="mb-4">
                ...
                <StudentAttendanceStudentDataTable
            ...
            <div className="-mx-2 flex flex-1 flex-col px-2">
              ...
              >
                <div className="flex flex-1 flex-col">
                  <Button disabled={!isUpcomingOrStarted} loading={isSubmitting} onClick={onSubmit}>
                    <BookUp2 />
                    Điểm danh
                  </Button>
                </div>
