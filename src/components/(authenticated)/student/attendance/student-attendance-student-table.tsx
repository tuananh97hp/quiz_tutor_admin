...
interface IStudentAttendanceStudentTableProps {
  isPending?: boolean;
  studentList: IStudentAttendance[];
  onChangeStdSelected: (_id: number) => void;
}

export const StudentAttendanceStudentDataTable = ({
  isPending,
  studentList,
  onChangeStdSelected,
}: IStudentAttendanceStudentTableProps) => {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          if (row.original.is_attendance) {
            return <Badge variant="default">Đi học</Badge>;
          }
