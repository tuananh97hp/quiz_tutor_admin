...
export const ClassExportForm = ({}: IClassFormProps) => {
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    try {
      const response = await ClassService.exportClass(currentSession?.accessToken, data);
      downloadBlobFile(response, 'classes.xlsx');
    } catch (e) {
      if (e instanceof AxiosError && e.status === HTTP_CODE.UNPROCESSABLE_ENTITY) {
        setErrorResponse(e, form.setError);
      }
      toast(`Failed To export excel Class`, { type: 'error' });
    }
  }
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <Select defaultValue={field.value} onValueChange={field.onChange}>
                                ...
                                <SelectContent>
                                  <SelectItem value={CLASS_STATUS.OPEN}>Hoạt động</SelectItem>
                                  <SelectItem value={CLASS_STATUS.CLOSE}>Đóng</SelectItem>
                                </SelectContent>
                              </Select>
                            </FormControl>
                            <FormMessage />
