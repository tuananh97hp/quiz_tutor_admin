...
interface IClassImportFormProps {}

export const ClassImportForm = ({}: IClassImportFormProps) => {
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      await ClassService.importClass(currentSession?.accessToken, formData);
      toast(`Successfully Class`, { type: 'success' });
      router.push('/admin/class');
    } catch (e) {
      if (e instanceof AxiosError && e.status === HTTP_CODE.UNPROCESSABLE_ENTITY) {
        setErrorResponse(e, form.setError);
      }
      toast(`Failed To Import excel Class`, { type: 'error' });
    }
  }
  ...
  async function handleDownloadTemplate() {
    if (!currentSession?.accessToken) return;
    try {
      const response = await ClassService.downloadImportTemplate(currentSession?.accessToken);
      downloadBlobFile(response, 'class_template.xlsx');
    } catch (e) {
          ...
          <div className="grid w-full grid-cols-12 gap-8">
            <div className="relative col-span-12 lg:col-span-8 xl:col-span-7">
              <Card className="relative border-2 rounded-xl before:rounded-xl bg-background">
                <CardHeader>
                  <CardTitle className="text-2xl">Thông tin lớp</CardTitle>
                  <CardDescription>
              ...
              <div className="dark:bg-background border-border bg-neutral-100 flex flex-col overflow-auto rounded-xl border px-4 py-6">
                <div className="-mx-2 flex flex-1 flex-col px-2">
                  ...
                  <p className="text-muted-foreground mt-2 text-sm">
                    Điền đẩy đủ thông tin cần thiết như tên, ID lớp, lớp và các thông tin chi tiết
                    khác để nhập lớp. Các thông tin có thể chỉnh sửa sau khi tạo.
                  </p>
