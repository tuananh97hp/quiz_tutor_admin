...
interface ITeacherImportFormProps {}

export const TeacherImportForm = ({}: ITeacherImportFormProps) => {
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      await TeacherService.importTeacher(currentSession?.accessToken, formData);
      toast(`Successfully Teacher`, { type: 'success' });
      router.push('/admin/teacher');
    } catch (e) {
      if (e instanceof AxiosError && e.status === HTTP_CODE.UNPROCESSABLE_ENTITY) {
        setErrorResponse(e, form.setError);
      }
      toast(`Failed To Import excel Teacher`, { type: 'error' });
    }
  }
  ...
  async function handleDownloadTemplate() {
    if (!currentSession?.accessToken) return;
    try {
      const response = await TeacherService.downloadImportTemplate(currentSession?.accessToken);
      downloadBlobFile(response, 'teacher_template.xlsx');
    } catch (e) {
              ...
              <div className="dark:bg-background border-border bg-neutral-100 flex flex-col overflow-auto rounded-xl border px-4 py-6">
                <div className="-mx-2 flex flex-1 flex-col px-2">
                  ...
                  <p className="text-muted-foreground mt-2 text-sm">
                    ...
                    khác để nhập hồ sơ giáo viên. Các thông tin có thể cập nhật sau khi tạo.
                  </p>
