...
export const ReferrerChangeStatus = ({ referrer, ...props }: IProps) => {
  const [status, setStatus] = React.useState<string>(referrer.status);
  ...
  const handleSave = async () => {
    try {
      ...
      await ReferrerService.updateReferrerStatus(currentSession.accessToken, referrer.id, {
        status,
      });
    ...
    <div>
      <Dialog {...props}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Thay đổi trạng thái giới thiệu viên [{referrer.name}]</DialogTitle>
            <DialogDescription>
