...
import { ReferrerChangeStatus } from '@/components/(authenticated)/admin/referrers/referrer-change-status';
import { useDisclosure } from '@/hooks/use-disclosure';
import Link from 'next/link';

export type IReferrerDataTableResult = FindResultSet<IReferrer>;

interface IReferrerTableProps {
  results: IReferrerDataTableResult;
}
interface IReferrerDataTableAction {
  referrer: IReferrer;
}

const ReferrerDataTableAction = ({ referrer }: IReferrerDataTableAction) => {
  const {
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-52" align="start" forceMount>
          ...
          <DropdownMenuItem asChild>
            <Link href={`/admin/referrer/${referrer.id}/edit`}>
              <Edit className="mr-2 h-4 w-4" /> Chỉnh sửa
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild>
            <Link href={`/admin/referrer/${referrer.id}/payment/create`}>
...
export const ReferrerDataTable = ({ results }: IReferrerTableProps) => {
  ...
  const columns = React.useMemo(() => {
    return [
      ...
      },
      {
        header: 'Hoạt động',
        cell: ({ row }) => {
          return <ReferrerDataTableAction referrer={row.original} />;
        },
        size: 70,
      },
      {
