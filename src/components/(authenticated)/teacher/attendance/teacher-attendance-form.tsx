...
import { BookUp2, ListCheck } from 'lucide-react';
import AttendanceService from '@/services/AttendanceService';
import { useSession } from 'next-auth/react';
import { toast } from 'react-toastify';
import { IClass, IStudentAttendance } from '@/types/models';
import _ from 'lodash';
import { TeacherAttendanceStudentDataTable } from '@/components/(authenticated)/teacher/attendance/teacher-attendance-student-table';
import { checkUpcomingOrStarted, currentScheduleOfDate } from '@/utils/models';
import AlertDialogConfirmed from '@/components/(authenticated)/teacher/attendance/alert-dialog-confirm';

interface ITeacherAttendanceFormProps {
  classItem: IClass;
}

export const TeacherAttendanceForm = ({ classItem }: ITeacherAttendanceFormProps) => {
  const { data: currentSession } = useSession();
  const currentSchedule = currentScheduleOfDate(classItem);
  const isUpcomingOrStarted = checkUpcomingOrStarted(currentSchedule);
  ...
  useEffect(() => {
    startTransitionPending(async () => {
      if (currentSession?.accessToken && classItem.id) {
        const results = await AttendanceService.teacherMeGetStudentAttendanceOfClass(
  ...
  function onSubmit() {
    startTransitionSubmit(async () => {
      ...
      try {
        ...
        await AttendanceService.teacherMeUpdateStudentAttendance(
          currentSession?.accessToken,
          classItem.id,
          {
          ...
          <Card className="relative border-2 rounded-xl before:rounded-xl bg-background">
            ...
            <CardContent className="p-2 pb-5">
              <div className="mb-4">
                ...
                <TeacherAttendanceStudentDataTable
          ...
          <div className="dark:bg-background border-border bg-neutral-100 sticky top-20 flex flex-col overflow-auto rounded-xl border px-4 py-6">
            <div className="-mx-2 flex flex-1 flex-col px-2">
              ...
              <p className="text-sm text-muted-foreground mb-2">
                Bạn sẽ được điểm danh 30 phút trước khi lớp học bắt đầu.
              </p>
              <div
                className={cn('custom-scrollbar -mx-2 flex flex-1 flex-col overflow-hidden px-2')}
              >
                <div className="flex flex-row gap-4">
                  <AlertDialogConfirmed classItem={classItem} isDisabled={!isUpcomingOrStarted} />
                  <Button
                    className="w-full"
                    disabled={!isUpcomingOrStarted}
