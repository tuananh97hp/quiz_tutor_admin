...
import { ArrowRight, Briefcase, Calendar, CalendarX, Clock, Users } from 'lucide-react';
...
import { currentScheduleOfDate, percentageSchedule, getStatusSchedule } from '@/utils/models';

interface IClassTableProps {
  classItem: IClass;
}

export const TeacherAttendanceClassItem = ({ classItem }: IClassTableProps) => {
    ...
    <div className="h-full">
      <Card className="border border-gray-200 bg-white rounded-xl relative flex flex-col h-full">
        <Badge className={`absolute top-4 right-4 text-sm font-medium px-4 py-1 rounded-full`}>
          {classItem.isConfirmed ? 'Đã Xác Nhận' : 'Đang Điểm Danh'}
        </Badge>
        ...
        <CardContent className="p-6 flex-1">
          ...
          <div className="space-y-3 text-sm pt-2">
            ...
            <p className="flex items-center gap-2 text-muted-foreground font-medium pt-2">
              ...
              Bắt đầu: <span className="">{currentSchedule?.start_time || '00:00'}</span>
              <CalendarX size={14} className="text-red-600 ml-4" />
              Kết thúc: <span className="">{currentSchedule?.end_time || '00:00'}</span>
            </p>
          </div>
        </CardContent>
            ...
            >
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button variant="outline" size="lg" className="transition-all duration-300" asChild>
                  <Link
                    href={`/teacher/attendance/class/${classItem.id}`}
