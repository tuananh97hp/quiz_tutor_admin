...
import TeacherClassesTodayItem from '@/components/(authenticated)/teacher/dashboard/teacher-classes-today-item';

interface ITeacherClassesTodayProps {
  classes: IClass[];
}

const TeacherClassesToday = ({ classes }: ITeacherClassesTodayProps) => {
  return (
        ...
        <ScrollArea className="h-80">
          {classes.length > 0 ? (
            classes.map((classItem, index) => (
              <TeacherClassesTodayItem key={index} classItem={classItem} />
            ))
          ) : (
            <div
...
export default TeacherClassesToday;
