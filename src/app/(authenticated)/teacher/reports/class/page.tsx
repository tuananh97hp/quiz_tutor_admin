...
import { TeacherReportClassDataTable } from '@/components/(authenticated)/teacher/reports/class/class-data-table';
...
const StudentReportsClassPage = async ({ searchParams }: IPageProps) => {
  ...
  const getTabHref = (tab: string) => {
    return getParamHref('/teacher/reports/class', searchParams, 'status', tab);
  };
  ...
  if (accessToken) {
    result = await classService.teacherMeGetClasses(accessToken, {
      status,
      search,
      page,
      per_page: perPage,
    });
    resultSummary = await classService.teacherMeGetClassesSummary(accessToken);
  }
  ...
  return (
    <div>
      ...
      <div className="mx-auto w-full max-w-screen-2xl">
        ...
        <div className="mt-4">
          {result?.data.length ? (
            <TeacherReportClassDataTable results={result} />
          ) : (
