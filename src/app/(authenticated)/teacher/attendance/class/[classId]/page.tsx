...
import { TeacherAttendanceForm } from '@/components/(authenticated)/teacher/attendance/teacher-attendance-form';
import Link from 'next/link';
import { ChevronLeftCircle } from 'lucide-react';
import { Metadata } from 'next';

export const metadata: Metadata = {
  title: 'Giáo Viên | Điểm Danh Lớp',
};
...
const CreateAttendancePage = async ({ params }: IAttendancePageProps) => {
  ...
  let classItem;
  try {
    classItem = await ClassService.teacherMeGetClassDetail(accessToken, Number(classId));
  } catch (e) {
    return notFound();
  }
    ...
    <div className="mx-auto w-full max-w-screen-2xl px-4 md:px-8">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-x-4 gap-y-8">
        ...
        <div className="flex w-full flex-col gap-4 p-1">
          <TeacherAttendanceForm classItem={classItem.data} />
        </div>
      </div>
    </div>
  );
