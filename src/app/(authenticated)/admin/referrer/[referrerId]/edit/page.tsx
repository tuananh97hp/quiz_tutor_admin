...
import { ReferrerForm } from '@/components/(authenticated)/admin/referrers/referrer-form';
import ReferrerService from '@/services/ReferrerService';
import { getCurrentAccessToken } from '@/utils/session';
import { Metadata } from 'next';

export const metadata: Metadata = {
  title: 'Admin | Chỉnh Sửa Giới Thiệu Viên',
};
...
interface IEditReferrerPageProps {
  params: IEditParams;
}
const EditReferrerPage = async ({ params }: IEditReferrerPageProps) => {
  ...
  let referrer;
  try {
    referrer = await ReferrerService.getReferrerDetail(accessToken, Number(referrerId));
  } catch (e) {
    return notFound();
  }
      ...
      <div className="mx-auto w-full max-w-screen-2xl px-4 md:px-8">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-x-4 gap-y-8">
          <div className="flex flex-row items-center">
            ...
            <h1 className="text-4xl font-semibold">Chỉnh Sửa Giới Thiệu Viên</h1>
          </div>
          <div className="flex w-full flex-col gap-4 p-1">
            <ReferrerForm referrer={referrer.data} />
          </div>
        </div>
      </div>
    </div>
